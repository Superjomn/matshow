...
from matshow.draw import Rectangle, Stack, Widget, colors, create_canvas
...
def test_draw_rectangle():
    ...
    rec0 = Rectangle(
        200, 200, fill=Widget.fill_hl_colors[1], outline=Widget.border_colors[1], border=2)
    rec1 = Rectangle(
...
def test_stack1():
    ...
    table0 = Stack([rec, rec, rec, rec, rec, rec], cstride=3,
    ...
    assert table0.inner_size == (
        rec.outer_size[0] * 3 + table0.border * 2, rec.outer_size[1] * 2 + table0.border * 2)
    assert table0.inner_size == table0.outer_size


def test_stack_with_margin():
    rec = Rectangle(
        40, 40, fill=Widget.fill_colors[0], outline=Widget.border_colors[0], border=6, margin=(10, 10))
    table0 = Stack([rec], cstride=4, border=40,
                   outline=colors.SGIBEET, fill=colors.WHITE)
    draw, canvas = create_canvas(table0.outer_size)
    table0.draw(draw)
...
def test_text():
    ...
    table1 = Stack([rec for i in range(20)], border=2,
                   outline=Widget.border_colors[0], cstride=10)
